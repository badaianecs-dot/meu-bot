...
const commands = [
  ...
  new SlashCommandBuilder()
    .setName("entrevista")
    .setDescription("Enviar mensagem de entrevista"),
].map((cmd) => cmd.toJSON());
...
client.on("interactionCreate", async (interaction) => {
  try {
    ...
    if (commandName === "atualizacoes") {
      ...
      if (textos.length === 0)
        return interaction.reply({ content: "❌ Informe pelo menos uma atualização.", ephemeral: true });
...
client.on("messageReactionAdd", async (reaction, user) => {
  try {
    ...
    // lógica de reações já existente aqui...
  } catch (err) {
    console.error(err);
  }
});
