...
client.on("interactionCreate", async (interaction) => {
  try {
    ...
    if (commandName === "cargostreamer") {
      const embed = new EmbedBuilder().setColor(COLOR_PADRAO).setTitle("Seja Streamer!")
      ...
      return interaction.editReply({ content: "✅ Cargo Streamer configurado!" });
    }
    ...
    if (!interaction.replied && !interaction.deferred) {
      ...
      try { await interaction.editReply({ content: "❌ Ocorreu um erro.", ephemeral: true }); } catch {}
    }
  }
});
